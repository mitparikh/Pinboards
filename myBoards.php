...
$con = mysqli_connect("localhost","root","root","pinboards");
	...
	$query = "select pictures.pid, boards.boardname 
				from boards join pin join pictures 
					where boards.boardid = pin.boardid and pin.pid = pictures.pid and boards.uname = '$username' 
...
if($result)
{
	...
	while($row = mysqli_fetch_array($result))
	{
		...
		echo "</br>";
		echo "<a href='myBoardView.php?boardname=".$boardname."&uname_of_owner_of_board=".$username."'><img src=showImage.php?pictureId=".$row['pid']." width=20% height=20%/></a>";
		//echo "</div>";
		echo "</hr>";
	}
}
?>
 		...
 		<form id="createboardButton" name = "createBoardButton" action="boardCreate.php" method="POST">
