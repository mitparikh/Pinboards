...
$boardname = $_POST['boardname'];
	...
	$query = "SELECT streamname FROM stream where uname = '$logged_in_username'"; 
	$result = mysqli_query($con, $query);
		
	?>	
	
	<form enctype="multipart/form-data" action="addToStreamQuery.php" method="post" name="streamForm">
		<br />
		Please Select the stream you want it to add it to:
		<br />
		...
		while ($row = mysqli_fetch_assoc($result)) 
		{
		    ...
		    echo "<option value =  " . $row['streamname'] . ">" . $row['streamname'] . "</option>";
		...
		<input type="hidden" name="username_of_owner_of_board" value="<?php echo "$owner_of_board"; ?>">
		<input type="submit" name="streamSubmit" value ="Add to stream" >
	</form>
	<br /> OR ENTER A NAME TO CREATE A NEW STREAM : 
	<form  action="addToStreamQuery.php" method="post" name="createStreamForm">
		<input type="text" name="createStreamName"/>
		<input type="hidden" name="boardname" value="<?php echo "$boardname"; ?>">
		<input type="hidden" name="username_of_owner_of_board" value="<?php echo "$owner_of_board"; ?>">
		<input type="submit"  value="Create a new Stream" />
	</form>
